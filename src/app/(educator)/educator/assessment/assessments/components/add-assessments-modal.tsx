...
interface AddAssessmentsModalProps {
...
// AddAssessmentsModal - create one or more assessments
export function AddAssessmentsModal({ onAddAssessments, trigger }: AddAssessmentsModalProps) {
  ...
  // handleSubmit - create assessment rows
  const handleSubmit = async (values: AssessmentFormSchema) => {
    ...
    try {
      ...
      toast.success(`${selections.length} assessment row${selections.length > 1 ? 's' : ''} created successfully.`)
      handleOpenChange(false)
    } catch (error) {
      toast.error(error instanceof Error ? error.message : 'Failed to create assessments.')
    }
  }
    ...
    <ResponsiveDialog open={open} onOpenChange={handleOpenChange}>
      ...
      <ResponsiveDialogContent showCloseButton={false} className="gap-0 p-0" desktopClassName="sm:max-w-[720px]">
        <ResponsiveDialogHeader className="border-b">
          <ResponsiveDialogTitle>Add New Assessment</ResponsiveDialogTitle>
          <ResponsiveDialogDescription>Create one assessment and save it to multiple subject rows.</ResponsiveDialogDescription>
        </ResponsiveDialogHeader>
                          ...
                          {assessmentCodeMode === 'preset' ? (
                            <FormField
                              ...
                              render={({ field: assessmentCodeField }) => (
                                <FormItem>
                                  <Select onValueChange={assessmentCodeField.onChange} value={assessmentCodeField.value}>
                                    <FormControl>
                    ...
                    <FormItem className="flex items-center justify-between rounded-md border p-4">
                      <div className="space-y-1">
                        ...
                        <p className="text-sm text-muted-foreground">
                          Enable shuffle for the selected assessments.
                        </p>
                      </div>
                      <FormControl>
                        <Switch
